...
if (!spots.length) spots.push({ name: '射座', pos: [8, 1.5, 64] });
...
for (const s of spots) {
  const out = await page.evaluate(`(() => {
    ...
    const from = new THREE.Vector3(${s.pos.join(',')});
    ...
    const rows = targets.map((t) => {
      ...
      return {
        ...
        遮り: hit ? +(hit.distance ?? hit.t ?? 0).toFixed(1) : null,
      };
    });
  ...
  for (const r of j.一覧) {
    ...
    if (ng || blocked) bad++;
    console.log(`  ${(r.名 ?? '').padEnd(6)} 距離 ${String(r.距離).padStart(5)}m`
