...
export function buildCompound(b) {
  ...
  /* ============ スポーン地点 ============ */
  /*
   * 以前は両陣営とも端に 6 点ずつ、1 か所に固めて置いていた。
   * マップの幅は 132m なので、湧くたびに必ず 110m 離れる。
   * 徒歩で 20〜25 秒かかり、そのあいだ試合は止まる。
   * 15 分回して撃破が 20 前後しか出ない最大の原因がこれだった。
   *
   * 実際の対戦マップは、陣営ごとに複数の湧き場を持ち、
   * 戦況に応じて前後する。ここでも後方・中間・前方の 3 段にし、
   * どこを使うかは Game.pickSpawn が敵味方の位置から選ぶ。
   *
   * 前方の湧き場は中央から 16m しか離れていないので、
   * 単純に「敵から遠い所」を選ぶと絶対に使われない。
   * 選び方のほうも合わせて直してある。
   */
  const spawnZones = [
    // [x, z, 何人ぶん]  A 陣営（東）。B 陣営は x を反転して作る
    [55, 0, 4],      // 後方
    [38, -15, 3],    // 中間（北寄り）
    [38, 15, 3],     // 中間（南寄り）
    [18, -13, 2],    // 前方（北）
  ...
  for (const [zx, zz, n] of spawnZones) {
    for (let i = 0; i < n; i++) {
      const dz = zz + (i - (n - 1) / 2) * 2.4;
      const dx = (i % 2) * 1.8;
      b.spawn('A', zx + dx, 0, dz, -Math.PI / 2);
      b.spawn('B', -zx - dx, 0, dz, Math.PI / 2);
    }
  }
