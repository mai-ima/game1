...
export function sectionRange(b, x0, cz) {
   ...
   * 土手は 1 枚の箱にしない。
   *
   * 高さ 4.4m の板を 1 枚立てただけでは、どの角度から見ても
   * 「緑がかった塀」にしか見えず、土を盛った物には見えなかった。
   * 手前を低く奥を高くした 3 段の台形で法面を作る。
   *
   * 段の材質は下から順に明るくする。
   * いちばん下を gravel にしたら、砂利のテクスチャが暗いうえに
   * 空を映すので、法面の足元が紺色の帯になった。
   */
  const bermSteps = [
    [0.0, 1.4, 0.0, 'dirt', SURFACE.DIRT],
    [1.2, 1.6, 1.1, 'sand', SURFACE.DIRT],
