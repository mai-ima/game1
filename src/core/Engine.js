...
export class Engine {
  ...
  render(dt) {
    ...
    /*
     * 「動きが無いフレームは影を描き直さない」も試したが、採らなかった。
     *
     * 影マップの描き直しはシーンをもう一度描くのと同じ費用なので、
     * 止められれば大きい。ところが対戦中はボットが常に動いていて
     * 一度も止められない。止まるのはメニュー・ローディング・ポーズだけで、
     * そこはもともとフレームレートが要らない場面。
     *
     * 一方で危険はある。影マップを一度も描かないまま
     * 材質がそれを読むと、読めた値が実装依存で画面が破綻する
     * （実際、メニューから入ると地上が真っ白になった）。
     * 得るものが小さく、失敗したときの壊れ方が大きいので入れない。
     *
     * 軽量モードの隔フレーム更新（上）は、必ず 2 フレームに 1 回は
     * 描くので同じ問題は起きない。
     */
