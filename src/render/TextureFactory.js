...
export class TextureFactory {
  ...
  constructor(renderer, opt = {}) {
     ...
     *
     * 1 組（アルベド・ORM・法線）は 2 か所に乗る。
     *   GPU  : size²×4×3 に、ミップマップぶんの 1.33 倍
     *   ヒープ: size²×4×3（DataTexture は元の配列を抱えたまま生きる）
     * 合わせて size²×4×3×2.34。512 なら 1 組 7.4MB、1024 なら 29.4MB。
     *
     * ヒープ側を数え落としていた時期があり、「420MB に収まっている」
     * という表示のまま実際は 1GB を超えて Chrome がタブごと落ちた。
     * 予算は必ず「実際に消費する合計」で数える。
     *
     * 材質は 119 種ある。見本ページは既定で 1024 の全種類を焼くので、
     * それだけで 3.5GB になる。機械の性能とは関係なく落ちる。
     * マップも材質を足すたびに同じ崖へ近づく。
     *
     ...
     */
    this.bytes = 0;
    this.budget = (opt.budget ?? 620) * 1024 * 1024;
    this.downgraded = 0;
    this.lastSize = 512;
  }
